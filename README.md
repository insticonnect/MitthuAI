...
- **Rules** — create categorization rules (app + title-contains → category). **A rule applies from now on**: every activity is categorized as it is captured, so days already recorded keep the labels they were given and writing a rule today never redraws last week's donut, focus score or Trends behind your back. Deleting a rule likewise only stops it matching new activity — the past is left alone. When re-labelling the past *is* what you meant, tick **also apply to past activity** on the form: that one rule then re-tags every matching day (plus same-title activity within ±10 minutes, so a video you flicked away from and back to lands in one bucket), skips anything you tagged by hand on the timeline, and reports how many events it moved. A backfill is one-way — deleting the rule afterwards does not undo it.
...
Because the label is written when the activity is captured, a rule changes what gets recorded next, not what was recorded already. One consequence is worth knowing: a Trends range that spans the day you wrote a rule will mix old and new labels, since the earlier days were labelled under the older rules. That is the price of a past that doesn't rewrite itself — use the rule form's **also apply to past activity** box when you'd rather have the whole range agree.
